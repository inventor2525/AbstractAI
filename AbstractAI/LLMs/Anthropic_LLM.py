...
class Anthropic_LLM(LLM):
	...
	def chat(self, conversation: Conversation, start_str: str = "", stream: bool = False, max_tokens: int = None) -> Message:
		with SafeStopwatch.singleton.scope():
			SafeStopwatch.singleton("Convert conversation to list")
			...
			if self.settings.rolling_cache:
				SafeStopwatch.singleton("Setup rolling cache")
			...
			SafeStopwatch.singleton("Anthropic hacks")
			...
			try:
				SafeStopwatch.singleton("Request")
				...
				print(e)
			if stream:
				SafeStopwatch.singleton("Stream setup")
				...
				SafeStopwatch.singleton("Return object setup")
